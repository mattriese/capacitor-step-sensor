...
class StepSensorDatabase private constructor(context: Context) :
    ...
    fun insertOrUpdate(bucketStart: Instant, bucketEnd: Instant, steps: Int) {
        ...
        // Upsert: insert new row, or add steps to existing bucket on conflict
        db.execSQL(
            """INSERT INTO $TABLE_NAME (bucket_start, bucket_end, steps, created_at)
               VALUES (?, ?, ?, ?)
               ON CONFLICT(bucket_start) DO UPDATE SET steps = MAX(steps, excluded.steps)""",
            arrayOf(startStr, endStr, steps, now)
        )
    }
    ...
    fun getStepsSince(since: Instant?): List<StepBucket> {
        ...
        val cursor = if (since != null) {
            ...
            db.rawQuery(
                "SELECT id, bucket_start, bucket_end, steps, created_at FROM $TABLE_NAME WHERE bucket_start >= ? ORDER BY bucket_start ASC",
                arrayOf(sinceStr)
            )
        } else {
            db.rawQuery(
                "SELECT id, bucket_start, bucket_end, steps, created_at FROM $TABLE_NAME ORDER BY bucket_start ASC",
                null
            )
        }
